...
from james_burvel_ocallaghan_iii import DefaultAioHttpClient, JamesBurvelOcallaghanIii, AsyncJamesBurvelOcallaghanIii
from james_burvel_ocallaghan_iii._utils import is_dict
...
logging.getLogger("james_burvel_ocallaghan_iii").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[JamesBurvelOcallaghanIii]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with JamesBurvelOcallaghanIii(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncJamesBurvelOcallaghanIii]:
