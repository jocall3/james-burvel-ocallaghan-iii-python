...
from james_burvel_ocallaghan_iii import BaseModel, JamesBurvelOcallaghanIii, AsyncJamesBurvelOcallaghanIii
...
from james_burvel_ocallaghan_iii._streaming import Stream
from james_burvel_ocallaghan_iii._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'james_burvel_ocallaghan_iii._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: JamesBurvelOcallaghanIii) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from james_burvel_ocallaghan_iii import BaseModel`",
    ):
        response.parse(to=PydanticModel)


@pytest.mark.asyncio
async def test_async_response_parse_mismatched_basemodel(async_client: AsyncJamesBurvelOcallaghanIii) -> None:
...
def test_response_parse_custom_stream(client: JamesBurvelOcallaghanIii) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=b"foo"),
        client=client,
        stream=True,
...
@pytest.mark.asyncio
async def test_async_response_parse_custom_stream(async_client: AsyncJamesBurvelOcallaghanIii) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=b"foo"),
        client=async_client,
        stream=True,
...
def test_response_parse_custom_model(client: JamesBurvelOcallaghanIii) -> None:
    response = APIResponse(
...
@pytest.mark.asyncio
async def test_async_response_parse_custom_model(async_client: AsyncJamesBurvelOcallaghanIii) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=async_client,
        stream=False,
...
def test_response_parse_annotated_type(client: JamesBurvelOcallaghanIii) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=client,
        stream=False,
...
def test_response_parse_bool(client: JamesBurvelOcallaghanIii, content: str, expected: bool) -> None:
    response = APIResponse(
...
def test_response_parse_expect_model_union_non_json_content(client: JamesBurvelOcallaghanIii) -> None:
    response = APIResponse(
